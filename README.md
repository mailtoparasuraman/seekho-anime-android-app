...
- Internet connection is available on the first launch to fetch initial anime data.
- Trailer videos and main cast information may not be available for all anime.
- Cached data stored locally is sufficient for offline browsing.
- Jikan API rate limits are respected and handled gracefully within the app.

---

## Known Limitations

- Pagination for the anime list is not implemented (limited to Top 25 anime).
- Search and filter functionality is not included.
- UI animations are kept minimal to focus on functionality.
- Unit and UI tests are not included due to time constraints.
...
- A debug APK is available in the `/apk` folder for quick testing.
- A short demo video can be provided upon request.
